defmodule YahooFinance.Quote do
  ...
  def get_full_quote(""), do: {:error, "Cannot provide empty string as argument"}

  def get_full_quote(symbol) when not is_string_like(symbol),
  ...
  def get_simple_quote(""), do: {:error, "Cannot provide empty string as argument"}

  def get_simple_quote(symbol) when not is_string_like(symbol),
    do: {:error, "Symbol argument must be given as string"}
  ...
  def get_custom_quote("", _), do: {:error, "Cannot provide empty string as argument"}

  def get_custom_quote(symbol, _args) when not is_string_like(symbol),
    do: {:error, "Symbol argument must be given as string"}

  def get_custom_quote(_symbol, args) when not is_list(args),
    do: {:error, "Arguments must be given as list"}

  def get_custom_quote(_symbol, []), do: {:error, "List arguments must not be empty"}
