defmodule YahooFinance.Historical do
  ...
  defp request_with_cookie_and_crumb(symbol) do
    ...
    |> HTTPoison.Retry.autoretry(max_attempts: 3, wait: 5000)
  end
  ...
  defp download_url(symbol, start_date, end_date, crumb) do
    "https://query1.finance.yahoo.com/v7/finance/download/#{symbol}?period1=#{start_date}&period2=#{
      end_date
  ...
  defp handle_error(301), do: {:error, "Invalid symbol given as argument"}

  defp handle_error(401), do: {:error, "Issue getting cookie - Invalid Cookie"}

  defp handle_error(400), do: {:error, "Issue getting API - Invalid Request"}

  defp handle_error(%{"finance" => info}), do: {:error, info["error"]["description"]}

  defp handle_error(%{"chart" => info}), do: {:error, info["error"]["description"]}
